...
public static class Template2ReportBuilder
{
    // Template 2: columns -> CustomerId | RemitterName | Amount | Order No | Trans Type | Date | Acct # | Category | Status
    // Changes from Template 1:
    // - InvoiceNumber -> OrderNumber (Order No)
    // - City -> TransactionType (Trans Type)
    // - Add TransactionDate (Date, 24-hr)
    // - Add AccountNumber (Acct #)
    // - CheckNumber -> Category
    // - New columns widths set to 40 (Date, Acct #). Category replaces Check # so width kept modest.
    public static byte[] Build(BatchDto batch, IReadOnlyList<PaymentDto> payments, string? templateName = null)
    {
        ...
        {
            40,   // Cust Id
            130,  // RemitterName (wrap)
            50,   // Amount
            50,   // Order No
            40,   // Type (TransactionType)
            80,   // Date (TransactionDate) - new
            40,   // Acct # (AccountNumber) - new
            40,   // Category (replacing CheckNumber)
            40    // Status
        };
        ...
        void DrawHeader()
        {
            ...
            gfx.DrawString("Cust Id", bold, XBrushes.Black, new XRect(colStarts[0], y + 4, colWidths[0], headerHeight), XStringFormats.TopLeft);
            ...
            gfx.DrawString("Order No", bold, XBrushes.Black, new XRect(colStarts[3], y + 4, colWidths[3], headerHeight), XStringFormats.TopLeft);
            gfx.DrawString("Type", bold, XBrushes.Black, new XRect(colStarts[4], y + 4, colWidths[4], headerHeight), XStringFormats.TopLeft);
            gfx.DrawString("Date", bold, XBrushes.Black, new XRect(colStarts[5], y + 4, colWidths[5], headerHeight), XStringFormats.TopLeft);
            gfx.DrawString("Acct #", bold, XBrushes.Black, new XRect(colStarts[6], y + 4, colWidths[6], headerHeight), XStringFormats.TopLeft);
            gfx.DrawString("Category", bold, XBrushes.Black, new XRect(colStarts[7], y + 4, colWidths[7], headerHeight), XStringFormats.TopLeft);
            gfx.DrawString("Status", bold, XBrushes.Black, new XRect(colStarts[8], y + 4, colWidths[8], headerHeight), XStringFormats.TopLeft);
        ...
        double bottomLimit() => page.Height - margin - footerHeight - 2;
        ...
        (string[] lines, double height) Wrap(string? text, double width)
        {
            ...
            foreach (var word in words)
            {
                ...
                if (size.Width <= width) current = next;
                else { if (!string.IsNullOrEmpty(current)) lines.Add(current); current = word; }
            }
            if (!string.IsNullOrEmpty(current)) lines.Add(current);
            return (lines.ToArray(), Math.Max(rowHeight, lines.Count * rowHeight));
        }
        ...
        foreach (var p in payments.OrderByDescending(p => p.Status).ThenBy(p => p.Id))
        {
            ...
            gfx.DrawString(p.OrderNumber ?? string.Empty, font, XBrushes.Black, new XRect(colStarts[3] + 2, y + 2, colWidths[3] - 4, height), XStringFormats.TopLeft);
            ...
            var dt = p.TransactionDate.HasValue ? p.TransactionDate.Value.ToString("yyyy-MM-dd HH:mm:ss") : string.Empty; // 24-hr
            gfx.DrawString(dt, font, XBrushes.Black, new XRect(colStarts[5] + 2, y + 2, colWidths[5] - 4, height), XStringFormats.TopLeft);

            gfx.DrawString(p.AccountNumber ?? string.Empty, font, XBrushes.Black, new XRect(colStarts[6] + 2, y + 2, colWidths[6] - 4, height), XStringFormats.TopLeft);
            gfx.DrawString(p.Category ?? string.Empty, font, XBrushes.Black, new XRect(colStarts[7] + 2, y + 2, colWidths[7] - 4, height), XStringFormats.TopLeft);
            gfx.DrawString(p.Status ?? string.Empty, font, XBrushes.Black, new XRect(colStarts[8] + 2, y + 2, colWidths[8] - 4, height), XStringFormats.TopLeft);
        ...
        if (y + 2 * rowHeight > bottomLimit()) NewPage();
        y += 12;
        ...
        for (int i = 0; i < totalPages; i++) DrawFooter(doc.Pages[i], i + 1, totalPages);
