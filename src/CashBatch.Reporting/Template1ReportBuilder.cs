...
public static class Template1ReportBuilder
{
    // Creates a PDF as a byte[] for the given batch and payments according to TemplateId=1
    public static byte[] Build(BatchDto batch, IReadOnlyList<PaymentDto> payments, string? templateName = null)
    {
        ...
        // Column widths (approximate; RemitterName is wider)
        var colWidths = new double[]
        {
            60,    // CustomerId
            125,   // RemitterName (wrap)
            70,    // Amount
            50,    // Invoice No
            80,    // City
            50,    // Check #
        ...
        void DrawHeader()
        {
            ...
            gfx.DrawString("City", bold, XBrushes.Black, new XRect(colStarts[4], y + 4, colWidths[4], headerHeight), XStringFormats.TopLeft);
            gfx.DrawString("Check #", bold, XBrushes.Black, new XRect(colStarts[5], y + 4, colWidths[5], headerHeight), XStringFormats.TopLeft);
            gfx.DrawString("Status", bold, XBrushes.Black, new XRect(colStarts[6], y + 4, colWidths[6], headerHeight), XStringFormats.TopLeft);
        ...
        void NewPage()
        {
            ...
            // NOTE: cannot reuse variable name after dispose; create new
        ...
        foreach (var p in payments.OrderByDescending(p => p.Status).ThenBy(p => p.Id))
        {
            ...
            gfx.DrawString(p.City ?? string.Empty, font, XBrushes.Black, new XRect(colStarts[4] + 2, y + 2, colWidths[4] - 4, height), XStringFormats.TopLeft);
            gfx.DrawString(p.CheckNumber ?? string.Empty, font, XBrushes.Black, new XRect(colStarts[5] + 2, y + 2, colWidths[5] - 4, height), XStringFormats.TopLeft);
            gfx.DrawString(p.Status ?? string.Empty, font, XBrushes.Black, new XRect(colStarts[6] + 2, y + 2, colWidths[6] - 4, height), XStringFormats.TopLeft);
        ...
        y += 12;
        // Draw label and value with only a space between
    ...
    private static void DrawFooter(PdfPage page, int pageNumber, int totalPages)
    {
        ...
        var bold = new XFont("Segoe UI", 11, XFontStyle.Bold);
